...
import Core.Room;
import Core.Suspect; // Assuming Suspect is in 'core'
import JsonDTO.CaseFile; // If loadSuspects method is in the same file
...
public class SuspectExtractor {
    ...
    public static void loadSuspects(CaseFile caseFile, GameContext context) throws NoValidRoomsException { // Make it throw NoValidRoomsException
        if (caseFile == null || caseFile.getSuspects() == null) {
            context.logLoadingMessage("Warning: No suspects defined in case file or case file is null for " + context.getContextIdForLog());
            return;
        }
        ...
        for (CaseFile.SuspectData suspectData : caseFile.getSuspects()) {
            ...
            if (suspectName == null || suspectName.trim().isEmpty()) {
                context.logLoadingMessage("Warning: Skipping suspect with null or empty name in " + context.getContextIdForLog());
                continue;
            }
            if (!suspectNames.add(suspectName.toLowerCase())) {
                context.logLoadingMessage("Warning: Duplicate suspect name '" + suspectName + "' found. Skipping duplicate in " + context.getContextIdForLog());
                continue;
            }
            ...
            try {
                Room startingRoom = assignRandomStartingRoom(suspect, context, context.getContextIdForLog());
                ...
                // If one suspect can't be placed, it might be okay to continue, or you might want to fail loading.
                // For now, let's log and re-throw if loadSuspects is declared to throw it.
                context.logLoadingMessage("Error placing suspect '" + suspect.getName() + "': " + e.getMessage() + " for " + context.getContextIdForLog());
     ...
     */
    private static Room assignRandomStartingRoom(Suspect suspect, GameContext context, String contextId)
            throws NoValidRoomsException {

        if (context == null || context.getAllRooms() == null) {
            throw new NoValidRoomsException("Game context or room list is null, cannot assign room for suspect: " + suspect.getName() + " in " + contextId);
        }
        ...
        if (allRoomsCollection.isEmpty()) {
            throw new NoValidRoomsException("No rooms available in context '" + contextId + "' to assign to suspect: " + suspect.getName());
        }
