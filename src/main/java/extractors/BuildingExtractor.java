...
import common.interfaces.GameContext; // Using the common interface
import Core.Room;
import JsonDTO.CaseFile;

import java.util.HashMap;
import java.util.HashSet;
import java.util.LinkedList; // For BFS queue
import java.util.Map;
import java.util.Queue; // For BFS queue
    ...
    public static boolean loadBuilding(CaseFile caseFile, GameContext context) {
        ...
        for (CaseFile.RoomData roomData : caseFile.getRooms()) {
            ...
            if (roomName == null || roomName.trim().isEmpty()) {
                context.logLoadingMessage("Warning: Skipping room with null or empty name in " + context.getContextIdForLog());
                hasErrors = true;
                continue;
            }
            if (!roomNamesProcessed.add(roomName.toLowerCase())) {
                context.logLoadingMessage("Warning: Duplicate room name '" + roomName + "' found. Skipping duplicate in " + context.getContextIdForLog());
                hasErrors = true;
                continue;
            }
            ...
            if (roomData.getNeighbors() != null) {
                for (Map.Entry<String, String> neighborEntry : roomData.getNeighbors().entrySet()) {
                    ...
                    if (!VALID_DIRECTIONS.contains(direction)) {
                        context.logLoadingMessage("Warning: Invalid direction '" + direction + "' for room '" + currentRoom.getName() + "'. Skipping neighbor link in " + context.getContextIdForLog());
                        hasErrors = true;
                        continue;
                    }
                    ...
                    if (neighborRoom != null) {
                        ...
                        context.logLoadingMessage("Warning: Neighbor room '" + neighborName + "' not found for room '" + currentRoom.getName() + "'. Skipping neighbor link in " + context.getContextIdForLog());
                        hasErrors = true;
                    }
                }
            }
        ...
        if (startingRoom == null) {
            context.logLoadingMessage("Critical Error: Starting room '" + caseFile.getStartingRoom() + "' not found or was invalid. Building load failed for " + context.getContextIdForLog());
        ...
        try {
            ...
            context.logLoadingMessage("Critical Error: Room connectivity validation failed: " + e.getMessage() + " for " + context.getContextIdForLog());
            return false; // Critical error
        }

        if (hasErrors) {
            context.logLoadingMessage("Building loaded with some non-critical errors/warnings for " + context.getContextIdForLog());
        } else {
            context.logLoadingMessage("Building structure loaded successfully for " + context.getContextIdForLog());
        }
    ...
    private static void validateRoomConnectivity(GameContext context, Room startRoom) {
        ...
        for (Room room : context.getAllRooms().values()) {
            if (!visited.contains(room)) {
                throw new IllegalStateException("Room '" + room.getName() + "' is unreachable from the starting room '" + startRoom.getName() + "'.");
            }
        }
    }
}