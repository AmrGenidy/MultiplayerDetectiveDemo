  ...
  protected void executeCommandLogic(GameActionContext context) {
    if (!context.canStartFinalExam(getPlayerId())) {
      context.sendResponseToPlayer(
          getPlayerId(), new TextMessage("You cannot start the final exam at this time.", true));
      return;
    }

    // Send the initial confirmation message (optional, as context might send its own)
    // context.sendResponseToPlayer(getPlayerId(), new TextMessage("Final exam initiated. Stand by
    // for questions...", false));
