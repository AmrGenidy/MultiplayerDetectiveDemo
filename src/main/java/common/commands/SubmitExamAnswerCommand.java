...
public class SubmitExamAnswerCommand extends BaseCommand {
  ...
  protected void executeCommandLogic(GameActionContext context) {
    ...
    // The context's implementation of processExamAnswer (in GameContextServer or
    // GameContextSinglePlayer)
    // is now responsible for:
    // 1. Storing the answer.
    // 2. Incrementing its internal question index.
    // 3. Calling its internal method to send the next question (e.g.,
    // sendNextExamQuestionToSession)
    //    OR calling its internal method to evaluate and send final results if all questions are
    // done.
  }
