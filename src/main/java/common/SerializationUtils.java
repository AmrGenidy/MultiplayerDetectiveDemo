...
public class SerializationUtils {
  ...
  /**
   * Writes a length-prefixed serialized object to a SocketChannel. This method is blocking for the
   * write operations but assumes the channel is ready. For non-blocking, this would be part of a
   * larger write handling loop.
   *
  ...
  /**
   * Reads a length-prefixed serialized object from a SocketChannel. This method attempts to read in
   * a blocking manner for simplicity here. In a true non-blocking server, this would be more
   * complex, involving accumulating bytes in a session-specific buffer until a full frame is
   * received.
   *
   ...
   * @throws ClassNotFoundException if the class of a serialized object cannot be found.
   */
  public static Object readFramedObject(SocketChannel channel)
    ...
    if (objectLength <= 0
        || objectLength > 1024 * 1024 * 10) { // Basic sanity check for length (e.g., max 10MB)
