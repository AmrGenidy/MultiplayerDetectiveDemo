...
/**
 * ClientState
 * Represents the different states the game client can be in.
 * This helps manage UI (what menus/prompts to show) and what actions are valid.
 */
public enum ClientState {
    ...
    REQUESTING_CASE_LIST_FOR_HOST, // Waiting for server to send available cases (after selecting host type).
    ...
    // General "Waiting for Server" State (if a more generic one is needed beyond specific SENDING_* states)
    // AWAITING_SERVER_RESPONSE, // Could be used for generic waits if specific SENDING_* states are too many.
