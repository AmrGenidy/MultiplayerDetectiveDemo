...
import Core.*; // Assuming all core classes are here: Room, Detective, Suspect, etc.
import JsonDTO.CaseFile;

import java.io.Serializable;
import java.util.*; // Use specific imports if preferred
...
public class GameContextSinglePlayer implements GameContext, GameActionContext {
    ...
    // awaitingAnswerForQuestionNumber removed, use isAwaitingExamAnswer() and getAwaitingQuestionNumber() helpers
    ...
    /**
     * Resets the context state in preparation for loading a new case.
     * Clears rooms, suspects, journal, resets flags and exam state.
     * Keeps the detective object but resets its internal state.
     */
    ...
    private void broadcastInitialCaseDetailsToPlayer(String playerId) {
        ...
        if (spCurrentRoom != null) {
            ...
            sendResponseToPlayer(playerId, createRoomDescriptionDTO(spCurrentRoom, playerId)); // Use existing helper
        } else {
    ...
    public void setCaseStarted(boolean started) {
        if (this.caseStarted == started && started) {
            logContextMessage("setCaseStarted called with " + started + ", but caseStarted is already " + this.caseStarted);
            return;
        }
        ...
        if (started) {
            ...
            broadcastInitialCaseDetailsToPlayer(this.detective.getPlayerId()); // Pass the SP detective's ID
        } else {
    ...
    public void processUpdateDisplayName(String playerId, String newDisplayName) {
        ...
        if (spDetective != null) {
            String oldName = spDetective.getPlayerId(); // Assuming getPlayerId() also acts as display name for SP detective
            ...
            logContextMessage("Display name update processed for " + oldName + " to " + newDisplayName +
    ...
    public void processRequestStartCase(String requestingPlayerId) {
        ...
        // The actual display of initial case info is handled by setCaseStarted -> broadcastInitialCaseDetails (SP version)
        // which implicitly targets the single player.
        setCaseStarted(true);
        // If setCaseStarted doesn't automatically display all initial info, you might need to trigger it here.
    ...
    public String examineObjectInPlayerRoom(String playerId, String objectName) {
        if (this.currentRoom == null) return "Error: Cannot examine, current location unknown.";
        GameObject object = this.currentRoom.getObject(objectName);
        if (object != null) {
            String result = object.getExamine();
            if (result == null || result.trim().isEmpty()) result = object.getDescription(); // Fallback
            return result; // Return just the description text
        }
    ...
    public String questionSuspectInPlayerRoom(String playerId, String suspectName) {
        if (this.currentRoom == null) return "Error: Cannot question, current location unknown.";
        Suspect suspect = getSuspectByName(suspectName);
        if (suspect != null && suspect.getCurrentRoom() != null &&
                suspect.getCurrentRoom().getName().equalsIgnoreCase(this.currentRoom.getName())) {
            return suspect.getStatement(); // Return just the statement
        }
        return "'" + suspectName + "' is not here or not found.";
    }

    @Override
    public String deduceFromObjectInPlayerRoom(String playerId, String objectName) {
        if (this.currentRoom == null) return "Error: Cannot deduce, current location unknown.";
        if (this.detective == null) return "Error: Player state not initialized.";

        GameObject object = this.currentRoom.getObject(objectName);
        if (object != null) {
            String lowerCaseObjName = object.getName().toLowerCase();
            if (this.detective.hasDeducedObject(lowerCaseObjName)) {
                return "You have already made a deduction about " + object.getName() + ".";
            }
            String clue = object.getDeduce();
            if (clue != null && !clue.trim().isEmpty()) {
                this.detective.incrementDeduceCount(lowerCaseObjName); // Use lower case consistently
                return clue; // Return just the clue
            }
            return "You ponder about " + object.getName() + " but gain no new insights.";
        }
        return "There is no '" + objectName + "' here to deduce from.";
    }
    @Override
    public void addJournalEntry(JournalEntryDTO entry) {
        boolean added = this.journal.addEntry(entry); // Add the entry
        if (added) {
            //logContextMessage("Journal entry added by " + entry.getContributorPlayerId());
        } else {
            //logContextMessage("Journal entry by " + entry.getContributorPlayerId() + " was duplicate, not added.");
        }
    }

    @Override
    public List<JournalEntryDTO> getJournalEntries(String playerId) {
        return this.journal.getEntries().stream()
    ...
    public void sendResponseToPlayer(String playerId, Serializable responseDto) {
        ...
        if (responseDto instanceof TextMessage) {
        ...
        } else if (responseDto instanceof RoomDescriptionDTO) {
            RoomDescriptionDTO rd = (RoomDescriptionDTO) responseDto;
            ...
            if (rd.getExits().isEmpty()) {
                ...
                rd.getExits().forEach((dir, roomName) -> sb.append(dir).append(" (").append(roomName).append("), "));
                if (!rd.getExits().isEmpty()) sb.setLength(sb.length() - 2);
            }
            output = sb.toString();
        } else if (responseDto instanceof JournalEntryDTO) {
            output = "Journal: " + responseDto.toString(); // Assumes JournalEntryDTO.toString() is good
        ...
        // Consider adding specific handlers for other DTOs if their default toString isn't ideal for player output
        // e.g., TaskListDTO, AvailableCasesDTO, PublicGamesListDTO if they are ever sent via sendResponseToPlayer in SP
        else {
            // Fallback for any other DTO types
            output = "[SP_RESPONSE] " + responseDto.toString();
        }
...
// Inside singleplayer/GameContextSinglePlayer.java
    ...
    @Override public List<CaseFile.ExamQuestion> getFinalExamQuestions() { return Collections.emptyList(); }
    @Override public int submitFinalExamAnswers(String playerId, Map<String, String> answers) { return 0; }
    @Override public void updatePlayerRank(String playerId) { this.detective.evaluateRank(); }
    ...
    public void updateNpcMovements(String triggeringPlayerId) {
        ...
        if (this.currentRoom == null) {
            //logContextMessage("NPC Movement SKIPPED: Player's current room is unknown (should not happen if case started).");
            return;
        }

        String playerCurrentRoomName = this.currentRoom.getName(); // Still useful for logging
        ...
        for (Suspect suspect : this.suspects) {
            ...
            String oldSuspectRoomName = suspect.getCurrentRoom().getName();
            Map<String, Room> neighbors = suspect.getCurrentRoom().getNeighbors();

            if (neighbors.isEmpty()) {
                //logContextMessage("Suspect " + suspect.getName() + " in " + oldSuspectRoomName + " has no neighbors, stays put.");
                continue;
            }
            ...
            if (!allPossibleMoves.isEmpty()) { // Should always be true if neighbors wasn't empty
                ...
                //logContextMessage("Suspect " + suspect.getName() + " moved RANDOMLY from " + oldSuspectRoomName + " to " + nextRoomForSuspect.getName() + ".");
            } else {
                // This case should ideally not be reached if neighbors was not empty.
                //logContextMessage("Suspect " + suspect.getName() + " in " + oldSuspectRoomName + " stays put (unexpected: had neighbors but list became empty).");
            }
        }

        // --- Move Watson (logic remains the same, can always enter player room, moves randomly) ---
        if (this.watson != null && this.watson.getCurrentRoom() != null) {
            String oldWatsonRoomName = this.watson.getCurrentRoom().getName();
            ...
            if (!watsonNeighbors.isEmpty()) {
                ...
                //logContextMessage("Dr. Watson moved RANDOMLY from " + oldWatsonRoomName + " to " + nextRoomForWatson.getName() + ".");
            } else {
                //logContextMessage("Dr. Watson in " + oldWatsonRoomName + " has no neighbors, stays put.");
            }
        } else {
            //logContextMessage("Dr. Watson is not available or not in a room, cannot move.");
        }
    ...
    public boolean wantsToExitToCaseSelection() { return wantsToExitToCaseSelection; }
    public boolean wantsToExitApplication() { return wantsToExitApplication; }
    public void resetExitFlags() { this.wantsToExitApplication = false; this.wantsToExitToCaseSelection = false; }

    // --- Getters/Setters used by SinglePlayerMain ---
    public void clearRooms() { this.rooms.clear(); }
    public void clearSuspects() { this.suspects.clear(); }
    public boolean isAwaitingExamAnswer() { return isExamActive && currentExamQuestionsList != null && currentQuestionIndex < currentExamQuestionsList.size(); }
    public int getAwaitingQuestionNumber() { return isAwaitingExamAnswer() ? currentQuestionIndex + 1 : 0; }
