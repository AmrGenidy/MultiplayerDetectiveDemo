...
import common.NetworkConstants; // For DEFAULT_PORT
import java.io.IOException;
import java.util.Scanner;
import java.util.NoSuchElementException; // Import for catching scanner exception
...
public class ServerMain {
    public static void main(String[] args) {
        ...
        Scanner consoleScanner = null; // Initialize outside try
        ...
        try {
            ...
            server.log("Server console ready. Type 'shutdown' to stop the server or 'saveall' to save games.");
            while (true) {
                String input = "";
                try {
                    if (System.in.available() > 0 || consoleScanner.hasNextLine()) { // Check if input might be available or if stream is open
                ...
                } else if ("reloadcases".equals(input)) {
