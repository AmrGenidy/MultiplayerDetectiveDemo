...
import common.commands.Command; // For executeCommand method signature (though not directly used in this file)
import Core.*; // Detective, Room, Suspect, DoctorWatson, TaskList, Journal, GameObject
import JsonDTO.CaseFile; // For selectedCase and ExamQuestion
...
public class GameContextServer implements GameContext, GameActionContext {
    ...
    private final CaseFile selectedCase;    // Made final as it shouldn't change post-construction for this context
    ...
    public GameContextServer(GameSession gameSession, CaseFile selectedCase, String p1Id, String p2Id) {
        ...
        // Player IDs can be null initially if P2 hasn't joined when context is first created by GameSession constructor
        this.player1Id = p1Id;
        this.player2Id = p2Id;
    ...
    public void setPlayerIds(String p1Id, String p2Id) {
        ...
        if (p2Id != null) {
            ...
            if (this.selectedCase != null && this.selectedCase.getStartingRoom() != null && this.player2Detective != null) {
                Room startingRoom = getRoomByName(this.selectedCase.getStartingRoom());
                if (startingRoom != null) {
                    this.player2Detective.setCurrentRoom(startingRoom);
                    logGameMessage("Player 2 (" + p2Id + ") position set to starting room: " + startingRoom.getName());
                } else {
                    logGameMessage("Warning: Could not set starting room for Player 2 (" + p2Id + ") upon ID set - starting room not found.");
                }
            }
        } else {
    ...
    public void resetForNewCaseLoad() {
        ...
        if (selectedCase.getWatsonHints() != null && !selectedCase.getWatsonHints().isEmpty()) {
            ...
            logGameMessage("Warning: No Watson hints found in selected case '" + selectedCase.getTitle() + "'.");
        }
    ...
    public void initializePlayerStartingState() {
        if (selectedCase.getStartingRoom() == null) {
            logGameMessage("CRITICAL Error: Cannot initialize player state, selected case has no startingRoom defined.");
            gameSession.endSession("Configuration error: No starting room defined."); // End session if critical
            return;
        }
        Room startingRoom = getRoomByName(selectedCase.getStartingRoom());
        if (startingRoom == null) {
            logGameMessage("CRITICAL Error: Starting room '" + selectedCase.getStartingRoom() + "' (defined in case) not found after loading rooms.");
            // Attempt to use any room as a fallback, but this is a severe config issue.
            if (!rooms.isEmpty()) {
                startingRoom = rooms.values().iterator().next();
                logGameMessage("Warning: Using first available room '" + startingRoom.getName() + "' as fallback starting room.");
            } else {
        ...
        if (!this.suspects.isEmpty() && !this.rooms.isEmpty()) {
            ...
            for (Suspect suspect : this.suspects) {
                List<Room> validSuspectStarts = allRoomsList.stream()
                        .filter(r -> !r.getName().equalsIgnoreCase(finalStartingRoom.getName())) // Try not to start in player room
    ...
    @Override public void addRoom(Room room) { if (room != null && room.getName() != null) rooms.put(room.getName().toLowerCase(), room); else logGameMessage("Error: Attempted to add null room or room with null name.");}
    @Override public Room getRoomByName(String name) { return name != null ? rooms.get(name.toLowerCase()) : null; }
    @Override public Map<String, Room> getAllRooms() { return Collections.unmodifiableMap(rooms); }
    @Override public void addSuspect(Suspect suspect) { if (suspect != null) suspects.add(suspect); else logGameMessage("Error: Attempted to add null suspect."); }
    @Override public void logLoadingMessage(String message) { logGameMessage("[LOADER] " + message); }
    @Override public String getContextIdForLog() { return "ServerSess-" + gameSession.getSessionId(); }

    // --- GameActionContext Implementation (for Commands) ---
    @Override public boolean isCaseStarted() { return caseStarted; }
...
// Inside server.GameContextServer.java
    ...
    public void setCaseStarted(boolean started) {
        ...
        if (this.caseStarted == started && started) {
            ...
            // Client's StartCaseCommand itself might handle this by checking context.isCaseStarted() first.
            return;
        }
        ...
        if (started) {
            logGameMessage("Case '" + (selectedCase != null ? selectedCase.getTitle() : "Unknown") + "' is being started.");
            ...
            if (this.gameSession != null) {
                ...
                logGameMessage("CRITICAL ERROR: gameSession is null in GameContextServer. Cannot update session state.");
            ...
            logGameMessage("Case '" + (selectedCase != null ? selectedCase.getTitle() : "Unknown") + "' has been stopped/reset (caseStarted=false).");
            // If stopping a case, also update GameSession's state if appropriate
            if (this.gameSession != null && this.gameSession.getState() == GameSessionState.ACTIVE) {
                // This path is less common, usually sessions end rather than case "stopping" mid-game
                // this.gameSession.setSessionState(GameSessionState.ENDED_NORMAL); // Or another appropriate state
            }
        }
    }
    ...
    private void broadcastInitialCaseDetails() {
        if (selectedCase == null) {
            ...
            broadcastToSession(new TextMessage("Critical error: Case data missing, cannot start.", true), null);
            return;
        }

        // 1. Broadcast Case Invitation:
        //    This is often sent by GameSession when both players join and are ready for 'start case' prompt.
        //    If 'start case' command is what TRULY starts the game flow, then invitation makes more sense here.
        ...
        broadcastToSession(new TextMessage("--- Case Description ---\n" + selectedCase.getDescription(), false), null);
        ...
        // Get the starting room based on player1 (could be player2, it's the same initial room for the case)
        Room startingRoom = null;
        if (player1Detective != null && player1Detective.getCurrentRoom() != null) { // Check if P1 detective and room are set
            startingRoom = player1Detective.getCurrentRoom();
        } else if (player2Detective != null && player2Detective.getCurrentRoom() != null) { // Fallback to P2 if P1 not fully init
            ...
            if(startingRoom != null && player1Detective != null) player1Detective.setCurrentRoom(startingRoom);
            if(startingRoom != null && player2Detective != null) player2Detective.setCurrentRoom(startingRoom);
        }


        if (startingRoom != null) {
            logGameMessage("Broadcasting starting location: " + startingRoom.getName());
            broadcastToSession(new TextMessage("\nYou are now at the starting location: " + startingRoom.getName(), false), null);
            ...
            List<String> objectNames = startingRoom.getObjects().values().stream().map(GameObject::getName).collect(Collectors.toList());

            List<String> occupantNamesForBroadcast = new ArrayList<>();
            // Player 1 (if in starting room - should be)
            if (player1Detective != null && player1Detective.getCurrentRoom() != null && player1Detective.getCurrentRoom().getName().equalsIgnoreCase(startingRoom.getName())) {
            ...
            if (player2Detective != null && player2Detective.getCurrentRoom() != null && player2Detective.getCurrentRoom().getName().equalsIgnoreCase(startingRoom.getName())) {
            ...
            for (Suspect s : suspects) {
                if (s.getCurrentRoom() != null && s.getCurrentRoom().getName().equalsIgnoreCase(startingRoom.getName())) {
                    occupantNamesForBroadcast.add(s.getName());
                }
            }
            if (watson != null && watson.getCurrentRoom() != null && watson.getCurrentRoom().getName().equalsIgnoreCase(startingRoom.getName())) {
                occupantNamesForBroadcast.add("Dr. Watson");
            }

            Map<String, String> exits = startingRoom.getNeighbors().entrySet().stream()
                    .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getName()));
            RoomDescriptionDTO initialRoomDTO = new RoomDescriptionDTO(
                    startingRoom.getName(),
                    startingRoom.getDescription(),
                    objectNames,
                    occupantNamesForBroadcast, // This is a general view, client 'look' might be more personalized
                    exits);

            broadcastToSession(initialRoomDTO, null);
        } else {
            logGameMessage("Error: Starting location could not be determined for broadcast.");
            broadcastToSession(new TextMessage("Error: Starting location not found for the case.", true), null);
        }

        logGameMessage("Broadcasting 'type help' message...");
        broadcastToSession(new TextMessage("\nType 'help' to see available commands.", false), null);
    }
    @Override public CaseFile getSelectedCase() { return selectedCase; }
    ...
    public String getOccupantsDescriptionInRoom(Room room, String askingPlayerId) {
        ...
        // Player 1
        if (player1Detective != null && player1Detective.getCurrentRoom() != null && player1Detective.getCurrentRoom().getName().equalsIgnoreCase(room.getName())) {
            if (player1Id == null || !player1Id.equals(askingPlayerId)) {
                occupantNames.add(p1Session != null ? p1Session.getDisplayId() : "Player 1 (Host)");
            }
        }
        // Player 2
        if (player2Detective != null && player2Detective.getCurrentRoom() != null && player2Detective.getCurrentRoom().getName().equalsIgnoreCase(room.getName())) {
        ...
        // Watson
        if (watson != null && watson.getCurrentRoom() != null && watson.getCurrentRoom().getName().equalsIgnoreCase(room.getName())) {
            occupantNames.add("Dr. Watson");
        }
        return occupantNames.isEmpty() ? "Occupants: None" : "Occupants: " + String.join(", ", occupantNames);
    }

    @Override public TaskList getTaskList() { return taskList; }
    @Override public DoctorWatson getWatson() { return watson; }
    @Override public List<Suspect> getAllSuspects() { return Collections.unmodifiableList(suspects); }

    @Override
    public Suspect getSuspectByName(String name) {
        if (name == null) return null;
        return suspects.stream().filter(s -> s.getName().equalsIgnoreCase(name.trim())).findFirst().orElse(null);
    }
    ...
    public boolean movePlayer(String playerId, String direction) {
        ...
        if (movingPlayer == null) {
            ...
            sendResponseToPlayer(playerId, new TextMessage("Error: Player context not found.", true)); // Send error to player
            return false;
        }
        Room oldRoom = movingPlayer.getCurrentRoom();
        if (oldRoom == null) {
            logGameMessage("Error: movePlayer called for detective not in a room (playerId: " + playerId + ")");
            sendResponseToPlayer(playerId, new TextMessage("Error: Your current location is unknown. Cannot move.", true));
            return false;
        }
        ...
        if (newRoom != null) {
            ...
            logGameMessage("Player " + playerId + " moved from " + oldRoom.getName() + " to " + newRoom.getName() + " (server state updated).");
            ...
            sendResponseToPlayer(playerId, new TextMessage("You can't move " + direction + " from " + oldRoom.getName() + ".", false));
            return false;
        }
    }
    ...
    private void sendRoomDescriptionToPlayer(String playerId, Room room) {
        ...
        Map<String, String> exits = room.getNeighbors().entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getName()));
        sendResponseToPlayer(playerId, new RoomDescriptionDTO(room.getName(), room.getDescription(), objectNames, occupantNamesList, exits));
    }


    @Override
    public String examineObjectInPlayerRoom(String playerId, String objectName) {
        // This method is now less used as ExamineCommand directly uses GameObject.getExamine()
        // and then sends a TextMessage. This method can serve as a helper if needed by other logic.
        Room currentRoom = getCurrentRoomForPlayer(playerId);
        if (currentRoom == null) return "Error: Not in a valid room.";
        GameObject obj = currentRoom.getObject(objectName);
        if (obj == null) return "There is no '" + objectName + "' here.";
        String result = obj.getExamine();
        if (result == null || result.trim().isEmpty()) result = obj.getDescription(); // Fallback
        return "You examine the " + obj.getName() + ": " + result;
    }

    @Override
    public String questionSuspectInPlayerRoom(String playerId, String suspectName) {
        // Similar to examine, QuestionCommand handles most of this.
        Room currentRoom = getCurrentRoomForPlayer(playerId);
        if (currentRoom == null) return "Error: Not in a valid room.";
        Suspect suspect = suspects.stream()
                .filter(s -> s.getName().equalsIgnoreCase(suspectName) && s.getCurrentRoom() != null && s.getCurrentRoom().getName().equalsIgnoreCase(currentRoom.getName()))
                .findFirst().orElse(null);
        if (suspect == null) return "'" + suspectName + "' is not in this room.";
        return suspect.getName() + " says: \"" + suspect.getStatement() + "\"";
    }

    @Override
    public String deduceFromObjectInPlayerRoom(String playerId, String objectName) {
        Detective player = getPlayerDetective(playerId);
        Room currentRoom = getCurrentRoomForPlayer(playerId);
        if (player == null || currentRoom == null) {
            logGameMessage("Error: deduceFromObjectInPlayerRoom missing player/room context for " + playerId);
            return "Error: Cannot perform deduction due to missing context.";
        }

        GameObject obj = currentRoom.getObject(objectName);
        if (obj == null) {
            return "There is no '" + objectName + "' here to deduce from.";
        }

        String objLowerName = obj.getName().toLowerCase();
        if (deducedObjectsInSession.contains(objLowerName)) {
            return "A deduction has already been made about the " + obj.getName() + " in this investigation. Check the journal.";
        }

        String clue = obj.getDeduce();
        if (clue == null || clue.trim().isEmpty()) {
            return "You ponder about the " + obj.getName() + " but gain no new insights.";
        }

        deducedObjectsInSession.add(objLowerName);
        sessionDeduceCount++;
        player.incrementDeduceCount(objLowerName); // Track individual attempt for rank purposes
        logGameMessage("Player " + playerId + " deduced from " + obj.getName() + ". Session deduce count: " + sessionDeduceCount);

        // Journal entry is added by the DeduceCommand after this method returns the clue.
        return "Deduction from " + obj.getName() + ": " + clue;
    }

    @Override
    public void addJournalEntry(JournalEntryDTO entry) {
        if (journal.addEntry(entry)) {
            logGameMessage("Journal entry added by " + entry.getContributorPlayerId() + ": " + entry.getText());
            broadcastToSession(entry, null); // Broadcast the new DTO to all players
            // Send confirmation only to the contributor
            sendResponseToPlayer(entry.getContributorPlayerId(), new TextMessage("Your note was added to the journal.", false));
        } else {
            sendResponseToPlayer(entry.getContributorPlayerId(), new TextMessage("Note was a duplicate and not added.", false));
        }
    }
    ...
    public void sendResponseToPlayer(String playerId, Serializable responseDto) {
        ...
        if (client != null) {
            ...
            logGameMessage("Error: Attempted to send DTO to null or disconnected client: " + playerId + ". DTO: " + responseDto.getClass().getSimpleName());
        }
    }
    ...
    public void notifyPlayerMove(String movingPlayerId, Room newRoom, Room oldRoom) {
        ...
        if (otherPlayerSession != null && movingPlayerSession != null && oldRoom != null && newRoom != null) {
            otherPlayerSession.send(new TextMessage(
                    movingPlayerSession.getDisplayId() + " moved from " + oldRoom.getName() + " to " + newRoom.getName() + ".",
                    false
            ));
        } else {
            // Log if some info was missing, helps debugging but don't crash
            // logGameMessage("Could not fully notify other player of move for " + movingPlayerId +
            //               ". OtherPlayer: " + (otherPlayerSession != null) +
            //               ", MovingPlayer: " + (movingPlayerSession != null) +
    ...
    public boolean canStartFinalExam(String playerId) {
        ...
        if (!conditionsMet) {
            logGameMessage("Exam conditions not met for player " + playerId + " (isCaseStarted: " + isCaseStarted() + ", examActive: " + examActiveForSession + ")");
        }
    ...
    public void startExamProcess(String playerId) { // playerId is the initiator
        if (!canStartFinalExam(playerId)) { // This now checks if initiator is host
            sendResponseToPlayer(playerId, new TextMessage("You cannot start the final exam at this time (not host or conditions not met).", true));
            return;
        }
        if (selectedCase.getFinalExam() == null || selectedCase.getFinalExam().isEmpty()) {
            broadcastToSession(new TextMessage("Error: No final exam questions configured for this case.", true), null);
            return;
        }
        ...
        logGameMessage("Interactive final exam initiated by host: " + hostDisplay + ". Sending first question to all.");
        broadcastToSession(new TextMessage("--- Final Exam Initiated by " + hostDisplay + " ---", false), null);
    ...
    public void processRequestStartCase(String requestingPlayerId) {
        logGameMessage("PROCESS_REQUEST_START_CASE: by PlayerId=" + requestingPlayerId + ", CaseStarted=" + isCaseStarted() + ", IsHost=" + isPlayerHost(requestingPlayerId));
        ...
        if (isPlayerHost(requestingPlayerId)) { // If host typed "request start case"
            sendResponseToPlayer(requestingPlayerId, new TextMessage("As host, you can directly use the 'start case' command.", false));
            return;
        }
        ...
        if (player1Id != null) { // Check if host (player1Id) is actually connected/present
            ...
            String requesterDisplay = (requestingPlayerSession != null) ? requestingPlayerSession.getDisplayId() : "Your partner ("+requestingPlayerId.substring(0,4)+"..)";

            // Send prompt to HOST (player1Id)
            logGameMessage("PROCESS_REQUEST_START_CASE: Sending prompt to host " + player1Id + " about request from " + requestingPlayerId);
            sendResponseToPlayer(player1Id, new TextMessage(requesterDisplay + " has requested to start the case. Type 'start case' to begin.", false));

            // Send confirmation to GUEST (requestingPlayerId)
            sendResponseToPlayer(requestingPlayerId, new TextMessage("Request sent to the host to start the case.", false));
        } else {
            logGameMessage("PROCESS_REQUEST_START_CASE: Host (player1Id) is null or not available. Cannot process request from " + requestingPlayerId);
            sendResponseToPlayer(requestingPlayerId, new TextMessage("The host is not currently available to start the case.", true));
        }
    }
    ...
    public void processRequestInitiateExam(String requestingPlayerId) {
        if (!isCaseStarted()) {
            sendResponseToPlayer(requestingPlayerId, new TextMessage("The case has not started yet. Cannot request exam.", true));
            return;
        }
        ...
        if (isPlayerHost(requestingPlayerId)) {
            sendResponseToPlayer(requestingPlayerId, new TextMessage("As host, you can directly use 'final exam' to initiate.", false));
            return;
        }
        // Guest is requesting
        if (player1Id != null) { // If host is present
            ClientSession requestingPlayerSession = gameSession.getClientSessionById(requestingPlayerId);
            String requesterDisplay = requestingPlayerSession != null ? requestingPlayerSession.getDisplayId() : "Your partner";

            sendResponseToPlayer(player1Id, new TextMessage(requesterDisplay + " has requested to start the final exam. Type 'final exam' to initiate.", false));
            sendResponseToPlayer(requestingPlayerId, new TextMessage("Request sent to host to initiate the final exam.", false));
            logGameMessage("Player " + requestingPlayerId + " requested final exam. Host " + player1Id + " notified.");
        } else {
            sendResponseToPlayer(requestingPlayerId, new TextMessage("Host is not available to start the exam.", true));
        }
    }


    private void sendNextExamQuestionToSession() {
        logGameMessage("SEND_NEXT_Q_TO_SESS: examActive=" + examActiveForSession +
                ", CurrentIndex=" + currentExamQuestionIndex +
                ", QuestionsListSize=" + (currentExamQuestionsList != null ? currentExamQuestionsList.size() : "NULL_LIST"));
        ...
        if (currentExamQuestionsList == null || currentExamQuestionsList.isEmpty()) {
            logGameMessage("SEND_NEXT_Q_TO_SESS: CRITICAL - currentExamQuestionsList is null or empty! Cannot proceed with exam.");
            // Attempt to end exam gracefully if possible, or send error to host
            if (player1Id != null) {
                sendResponseToPlayer(player1Id, new TextMessage("Error: Exam questions are missing. Cannot continue exam.", true));
            }
        ...
        if (currentExamQuestionIndex < currentExamQuestionsList.size()) {
            ...
            ExamQuestionDTO questionDTO = new ExamQuestionDTO(currentExamQuestionIndex + 1, q.getQuestion());

            logGameMessage("SEND_NEXT_Q_TO_SESS: Broadcasting Q_DTO for Q" + (currentExamQuestionIndex + 1) + ": \"" + q.getQuestion().substring(0, Math.min(20, q.getQuestion().length())) + "...\"");
            broadcastToSession(questionDTO, null);

            // Send specific prompt to host
            if (player1Id != null) {
                logGameMessage("SEND_NEXT_Q_TO_SESS: Sending prompt to host " + player1Id + " for Q" + (currentExamQuestionIndex + 1));
                sendResponseToPlayer(player1Id, new TextMessage("Host, please submit your answer for Q" + (currentExamQuestionIndex + 1) + ".", false));
            }
            // Notify guest
            if (player2Id != null && (player1Id == null || !player2Id.equals(player1Id))) { // Ensure guest is not also host (single player test)
                ClientSession hostSess = gameSession.getClientSessionById(player1Id);
                String hostDisp = (hostSess != null) ? hostSess.getDisplayId() : (player1Detective != null ? player1Detective.getPlayerId() : "The Host");
                logGameMessage("SEND_NEXT_Q_TO_SESS: Notifying guest " + player2Id + " that host is answering Q" + (currentExamQuestionIndex + 1));
                sendResponseToPlayer(player2Id, new TextMessage(hostDisp + " is answering exam question " + (currentExamQuestionIndex + 1) + "/" + currentExamQuestionsList.size() + "...", false));
            }
        } else {
            // All questions have been presented and answers collected
            logGameMessage("SEND_NEXT_Q_TO_SESS: All questions (" + currentExamQuestionsList.size() + ") presented. CurrentIndex=" + currentExamQuestionIndex + ". Calling evaluateAndBroadcastExamResults for host: " + player1Id);
    ...
    public void processExamAnswer(String submittingPlayerId, int questionNumber, String answerText) {
        logGameMessage("PROCESS_EXAM_ANSWER: Initiated by PlayerId=" + submittingPlayerId +
                ", For QNum=" + questionNumber + ", Answer=\"" + answerText +
                "\", CurrentIndexBefore=" + currentExamQuestionIndex +
                ", ExamActive=" + examActiveForSession);

        if (!examActiveForSession) {
            logGameMessage("PROCESS_EXAM_ANSWER: Exam not active. Sending error to " + submittingPlayerId);
        ...
        if (!(player1Id != null && player1Id.equals(submittingPlayerId))) {
            logGameMessage("PROCESS_EXAM_ANSWER: Attempt to submit answer by non-host " + submittingPlayerId + ". Denied.");
            sendResponseToPlayer(submittingPlayerId, new TextMessage("Only the host can submit exam answers.", true));
            return;
        }
        ...
        if (questionNumber != (currentExamQuestionIndex + 1)) {
            logGameMessage("PROCESS_EXAM_ANSWER: Host " + submittingPlayerId + " submitted answer for Q" + questionNumber +
                    ", but server expected Q" + (currentExamQuestionIndex + 1) + ". Resending current question.");
            sendResponseToPlayer(submittingPlayerId, new TextMessage("Error: Answer submitted for an unexpected question. Please answer the current one.", true));

            // Resend the current expected question to the host to get them back on track
            if (currentExamQuestionsList != null && currentExamQuestionIndex < currentExamQuestionsList.size()) {
                CaseFile.ExamQuestion q = currentExamQuestionsList.get(currentExamQuestionIndex);
                logGameMessage("PROCESS_EXAM_ANSWER: Resending Q" + (currentExamQuestionIndex + 1) + " to host " + player1Id);
                sendResponseToPlayer(player1Id, new ExamQuestionDTO(currentExamQuestionIndex + 1, q.getQuestion()));
                sendResponseToPlayer(player1Id, new TextMessage("Host, please re-submit your answer for Q" + (currentExamQuestionIndex + 1) + ".", false));
            } else {
                logGameMessage("PROCESS_EXAM_ANSWER: Cannot resend question, list empty or index out of bounds. CurrentIndex=" + currentExamQuestionIndex);
            }
            return;
        }

        if (player1ExamAnswersMap == null) { // Should have been initialized in startExamForPlayer
            logGameMessage("PROCESS_EXAM_ANSWER: CRITICAL - player1ExamAnswersMap is null! Re-initializing.");
        ...
        logGameMessage("PROCESS_EXAM_ANSWER: Stored answer for Q" + questionNumber + " by host " + submittingPlayerId +
                ". MapSize=" + player1ExamAnswersMap.size() +
                ", CurrentIndexAfterIncrement=" + currentExamQuestionIndex +
    ...
    private void evaluateAndBroadcastExamResults(String hostPlayerId) {
        logGameMessage("EVAL_EXAM_RESULTS: Starting evaluation for host " + hostPlayerId + " in session " + gameSession.getSessionId());

        if (currentExamQuestionsList == null) {
            logGameMessage("EVAL_EXAM_RESULTS: Error - currentExamQuestionsList is null. Cannot evaluate.");
        ...
        if (player1ExamAnswersMap == null) {
            logGameMessage("EVAL_EXAM_RESULTS: Warning - player1ExamAnswersMap is null. Assuming 0 score.");
            player1ExamAnswersMap = new HashMap<>();
        }

        int score = 0;
        List<String> reviewableAnswersDetails = new ArrayList<>(); // This list will be sent to the client
        ...
        for (int i = 0; i < currentExamQuestionsList.size(); i++) {
            ...
            if (hostAnswer != null && hostAnswer.equalsIgnoreCase(correctAnswer)) {
                ...
                String reviewDetail = String.format("Q: %s\n  Your Answer: '%s'", // Removed "Correct Answer: '%s'"
                        actualQuestionText,
                        (hostAnswer != null ? hostAnswer : "Not answered")
                );
                // --- END MODIFICATION ---
                reviewableAnswersDetails.add(reviewDetail);
            }
        }
        logGameMessage("EVAL_EXAM_RESULTS: Host " + hostPlayerId + " score: " + score + "/" + currentExamQuestionsList.size());
        ...
        String hostDisplay = (hostSession != null) ? hostSession.getDisplayId() : (hostDetective != null ? hostDetective.getPlayerId() : "The Host");

        if (score == totalQuestions) feedback = "Outstanding! The case is solved perfectly by " + hostDisplay + "!";
        else if (score >= totalQuestions * 0.5) feedback = "Good effort by " + hostDisplay + "! Key aspects uncovered.";
        else feedback = "The mystery remains largely unsolved by " + hostDisplay + ". Further investigation was needed.";
        ...
        ExamResultDTO resultDTO = new ExamResultDTO(
                ...
                feedback,
                finalRankString,
                reviewableAnswersDetails // This list now only contains question and player's wrong answer
        );
        logGameMessage("EVAL_EXAM_RESULTS: Broadcasting ExamResultDTO: Score=" + score + ", Rank=" + finalRankString);
    ...
    private void resetServerExamState() {
        ...
        this.player1ExamAnswersMap = null;
        this.currentExamQuestionIndex = 0;
        logGameMessage("Server-side exam state has been reset for session " + gameSession.getSessionId());
    }
    ...
    public void processUpdateDisplayName(String playerId, String newDisplayName) {
        ...
        if (client != null) {
            ...
            if (newDisplayName != null && !newDisplayName.equals(oldDisplayName) && !newDisplayName.trim().isEmpty() && newDisplayName.length() < 25) {
                client.setDisplayId(newDisplayName); // Update on the ClientSession object
                logGameMessage("Player " + playerId + " (formerly " + oldDisplayName + ") changed display name to " + newDisplayName);

                // Broadcast the change to all players in the session
                PlayerNameChangedDTO pncDTO = new PlayerNameChangedDTO(playerId, oldDisplayName, newDisplayName);
                broadcastToSession(pncDTO, null); // Send to all, including the changer for confirmation sync
                ...
                sendResponseToPlayer(playerId, new TextMessage("Your display name is already " + newDisplayName + ".", false));
            } else {
                sendResponseToPlayer(playerId, new TextMessage("New display name is invalid or too long.", true));
            }
        } else {
    ...
    // Methods from GameActionContext that are now part of the interactive exam flow:
    @Override public List<CaseFile.ExamQuestion> getFinalExamQuestions() { /* Not directly called by commands now */ return Collections.emptyList(); }
    @Override public int submitFinalExamAnswers(String playerId, Map<String, String> answers) { /* Not directly called by commands now */ return 0;}
    @Override public void updatePlayerRank(String playerId) { /* Rank updated during exam eval */ }
    ...
    public String askWatsonForHint(String playerId) {
        ...
        if (watsonRoom.getName().equalsIgnoreCase(playerRoom.getName())) {
            ...
            addJournalEntry(new JournalEntryDTO("Dr. Watson's insight: " + hint, "Dr. Watson (Shared)", System.currentTimeMillis()));
            return "Watson: \"" + hint + "\"";
        }
    ...
    public void updateNpcMovements(String triggeringPlayerId) {
        ...
        String p1RoomName = (player1Detective != null && player1Detective.getCurrentRoom() != null) ? player1Detective.getCurrentRoom().getName() : "N/A";
        String p2RoomName = (player2Detective != null && player2Detective.getCurrentRoom() != null) ? player2Detective.getCurrentRoom().getName() : "N/A";
        logGameMessage("NPC Movement START (MP). Player1 in: " + p1RoomName + ", Player2 in: " + p2RoomName);
        ...
        for (Suspect suspect : this.suspects) {
            ...
            if (!allPossibleMoves.isEmpty()) {
                ...
                if (!newSuspectRoom.getName().equalsIgnoreCase(oldSuspectRoom.getName())) { // Check if actually moved
                    suspect.setCurrentRoom(newSuspectRoom);
                    logGameMessage("Suspect " + suspect.getName() + " moved RANDOMLY from " + oldSuspectRoom.getName() + " to " + newSuspectRoom.getName() + " (MP).");
                    broadcastToSession(new NpcMovedDTO(suspect.getName(), oldSuspectRoom.getName(), newSuspectRoom.getName()), null);
                }
            } else {
                logGameMessage("Suspect " + suspect.getName() + " in " + oldSuspectRoom.getName() + " stays put (MP - unexpected: had neighbors but list empty).");
            }
        }
        ...
        if (this.watson != null) {
            ...
            if (oldWatsonRoom == null) { // Should be initialized
                if (!rooms.isEmpty()) {
                    this.watson.setCurrentRoom(new ArrayList<>(rooms.values()).get(random.nextInt(rooms.size())));
                    logGameMessage("Watson was roomless (MP), placed randomly in " + this.watson.getCurrentRoom().getName());
            ...
            if (!watsonNeighbors.isEmpty()) {
                ...
                if (!newWatsonRoom.getName().equalsIgnoreCase(oldWatsonRoom.getName())) { // Check if actually moved
                    this.watson.setCurrentRoom(newWatsonRoom);
                    logGameMessage("Dr. Watson moved RANDOMLY from " + oldWatsonRoom.getName() + " to " + newWatsonRoom.getName() + " (MP).");
                    broadcastToSession(new NpcMovedDTO("Dr. Watson", oldWatsonRoom.getName(), newWatsonRoom.getName()), null);
                }
            } else {
                logGameMessage("Dr. Watson in " + oldWatsonRoom.getName() + " has no neighbors, stays put (MP).");
            }
        } else {
    ...
    @Override public String getContextId() { return "GameSessCtx-" + gameSession.getSessionId(); }
    ...
    public void executeCommand(Command command) { // This is the method called by GameSession
        ...
        if (command.getPlayerId() == null) { /* ... */ return; }

        logGameMessage("Context executing command: " + command.getClass().getSimpleName() + " for player " + command.getPlayerId());

        // --- HOST CHECKS ---
        if (command instanceof StartCaseCommand) {
            if (!isPlayerHost(command.getPlayerId())) {
                sendResponseToPlayer(command.getPlayerId(), new TextMessage("Only the host can directly start the case. Guests can use 'request start case'.", true));
                return;
            }
        } else if (command instanceof InitiateFinalExamCommand) {
            if (!isPlayerHost(command.getPlayerId())) {
                sendResponseToPlayer(command.getPlayerId(), new TextMessage("Only the host can directly initiate the final exam. Guests can use 'request final exam'.", true));
                return;
            }
        }
        // --- END HOST CHECKS ---
    ...
    public GameStateData getGameStateForSaving() {
        ...
        if (player1Detective != null && player1Detective.getCurrentRoom() != null) playerPos.put(player1Id, player1Detective.getCurrentRoom().getName());
        if (player2Detective != null && player2Detective.getCurrentRoom() != null) playerPos.put(player2Id, player2Detective.getCurrentRoom().getName());
        state.setPlayerCurrentRoomNames(playerPos);

        Map<String, String> npcPos = new HashMap<>();
        if (watson != null && watson.getCurrentRoom() != null) npcPos.put("Watson", watson.getCurrentRoom().getName());
        for (Suspect s : suspects) {
    ...
    public void applyGameState(GameStateData loadedState) {
        ...
        if (watson != null && loadedState.getNpcCurrentRoomNames() != null && loadedState.getNpcCurrentRoomNames().containsKey("Watson")) {
            watson.setCurrentRoom(getRoomByName(loadedState.getNpcCurrentRoomNames().get("Watson")));
        }
        for (Suspect s : suspects) { // Suspects list should be repopulated by extractors based on selectedCase
            if (loadedState.getNpcCurrentRoomNames() != null && loadedState.getNpcCurrentRoomNames().containsKey(s.getName())) {
        ...
        if (player1Detective != null && loadedState.getPlayerCurrentRoomNames() != null && loadedState.getPlayerCurrentRoomNames().containsKey(player1Id)) {
            player1Detective.setCurrentRoom(getRoomByName(loadedState.getPlayerCurrentRoomNames().get(player1Id)));
            player1Detective.setFinalExamScore(loadedState.getPlayerScores() != null ? loadedState.getPlayerScores().getOrDefault(player1Id, 0) : 0);
            player1Detective.evaluateRank();
        }
        if (player2Detective != null && loadedState.getPlayerCurrentRoomNames() != null && loadedState.getPlayerCurrentRoomNames().containsKey(player2Id)) {
            player2Detective.setCurrentRoom(getRoomByName(loadedState.getPlayerCurrentRoomNames().get(player2Id)));
            player2Detective.setFinalExamScore(loadedState.getPlayerScores() != null ? loadedState.getPlayerScores().getOrDefault(player2Id, 0) : 0);
